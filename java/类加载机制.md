...
加载tomcat /common/* 类库，可以被Tomcat和所有应用使用。
...
使用线程上下文类加载器(Thread Context ClassLoader)。在ThreadLocal中保存线程ClassLoader，在父加载器中可以使用ThreadLocal来获取子加载器。
